...
Path(r"D:/ai/carpet-fga/_smoke_1.16_1.20.6_fix2.ps1").write_text(r'''$ErrorActionPreference = "Continue"
...
$summary = Join-Path $logDir "summary-1.16-1.20.6-fix2.txt"
...
foreach ($v in $versions) {
  ...
  Write-Host "==== SMOKE $v ===="
  $log = Join-Path $logDir ($v + "-smoke-fix2.log")
  ...
  Set-Content -Path (Join-Path $runDir "server.properties") -Value @(
    "online-mode=false","max-players=5","spawn-protection=0","view-distance=2","simulation-distance=2",
    ("motd=FGA smoke " + $v),"level-name=world","gamemode=survival","difficulty=peaceful","enable-command-block=true"
  ) -Encoding ascii

  $arg = "/c `"`"" + $root + "\gradle-local.bat`" --no-daemon --configure-on-demand --max-workers=1 :" + $v + ":compileJava :" + $v + ":runServer --args=`"--port 0`" > `"" + $log + "`" 2>&1`""
  ...
  while ((Get-Date) -lt $deadline) {
    ...
    if (Test-Path $log) {
      ...
      if ($raw -and ($raw -match "BUILD FAILED|FAILURE: Build failed|Critical injection failure|Mixin prepare failed|Mixin apply for mod carpet-fga|InvalidMixinException|Could not resolve") -and ($raw -notmatch "Done \([0-9.]+s\)!")) {
        $status = "FAIL"; break
      }
    }
    if ($p.HasExited) {

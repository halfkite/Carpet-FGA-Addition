...
### 夺舍时交换权限(permissionSwapsToo) · [相关指令](commands.md#cmd-control-player)

true 使用操控者自己的命令权限<br>
false 使用被操控者的命令权限<br>
该规则只影响命令权限，不改变夺舍入口权限、存档归属或离线玩家数据
...
自定义 `/player` 名称参数的假人预设补全<br>
false：保留 Carpet 默认的 Steve 和 Alex<br>
填写 `[名称,名称]` 后替换默认预设，并保留在线玩家与夺舍目标补全<br>
仅影响提示，不限制手动输入

- 类型：`字符串`
- 默认值：`false`
- 参考选项：`false`、`[]`、`[bot_,hsds]`
- 分类：`FGA`、`特性`
- 生效版本：`1.21+`
...
允许多人游戏服务器接纳超过 `server.properties` 中 `max-players` 的玩家，不改变封禁、白名单和 IP 封禁检查<br>
检测到 GCA 已加载且 `fakePlayerResident` 开启时自动生效

- 类型：`布尔`
- 默认值：`false`
- 参考选项：`false`、`true`
- 分类：`FGA`、`特性`
- 生效版本：`1.21.1`、`1.21.3`、`1.21.4`、`1.21.5`、`1.21.8`、`1.21.10`、`1.21.11`、`26.1.2`、`26.2`、`26.3`
...
开启后提供两种无序配方：8 个金锭加 1 个胡萝卜，或 7 个金锭加 1 个普通金胡萝卜<br>
合成带有附魔外观的附魔 金胡萝卜，可在饱食度满时食用，食用后饱食度与饱和度均恢复到 20
...
百分百催熟树苗类作物、竹子、海泡菜等有概率一个骨粉生长多个阶段的作物百分百生长多个阶段
...
带有自定义名称的末影珍珠会将传送目标改为同名的在线玩家，找不到同名玩家时该末影珍珠不执行传送
...
false 或 0 保持原版上限；直接输入数字 N 让附魔的原版等级上限增加 N，最高保存为255级；水下速掘、精准采集、火矢、无限、引雷、多重射击、经验修补、绑定诅咒和消失诅咒始终保持 I 级
...
挖掘工具可以附魔火焰附加、火焰附加1会使掉落物烧炼1次、火焰附加2则是两次；工具不会在附魔台上获得火焰附加
...
开启后灵魂疾行在灵魂沙、灵魂土等兼容方块上移动时不会消耗靴子耐久
...
让村民在交易过程中达到经验门槛后立即升级，并同步交易界面的等级、经验和交易列表
...
### 村民售价最低(villagerMinimumTradePrice)

村民生成交易时使用原版随机售价的最低值，同时保留需求、声望和英雄奖励修正，仅在无职业村民重新绑定职业时生效
...
### 村民只售最高级附魔书(villagerOnlyMaxEnchantmentBooks)

村民新生成的附魔书会把抽到的附魔直接提升到目标等级<br>
onlyvanilla：提升到附魔书交易可达到的原版最高等级<br>
more：允许提升到 `enchantmentLevelLimitIncrease` 设置的等级<br>
仅影响村民重新获得职业或升级时新生成的交易

- 类型：`枚举`
- 默认值：`false`
- 参考选项：`false`、`onlyvanilla`、`more`
- 分类：`FGA`，`特性`
- 生效版本：`1.21.1-26.3`

### 村民只售最高级附魔武器装备(villagerOnlyMaxEnchantmentEquipment)

村民新生成的附魔武器和装备会把抽到的附魔直接提升到目标等级<br>
onlyvanilla：按原版装备交易的 5 至 19 附魔强度和物品附魔能力计算最高等级，不会把效率、锋利等附魔直接提升到原版装备交易无法达到的 V 级<br>
more：允许提升到 `enchantmentLevelLimitIncrease` 设置的等级<br>
未附魔装备和其他交易不受影响，仅影响新生成的交易
...
### 猪灵交易物品自定义(piglinBarterItemExclusions) · [相关指令](commands.md#cmd-piglin-barter-customization)

开启后通过 `/fga piglinBarterItemExclusions list` 查看并编辑当前猪灵交易战利品表<br>
可以通过点击列表内容进行编辑，概率总和不足 100% 时剩余部分为空交易，超过 100% 时按比例分配
...
开启后可将信标、荧石、南瓜灯、海晶灯、蛙明灯、末地烛、火把、蜡烛等发光方块放入切石机并切成亮度等级 1 至 15 的 `minecraft:light` 光源方块，每次产出 4 个；`1.21.9+` 额外支持铜火把和各种氧化、涂蜡铜灯笼<br>
任意亮度等级的 `minecraft:light` 光源方块也可以在切石机中互相一比一转换
...
ops：需要 OP 2 及以上<br>
0-4：设置命令的最低权限等级
使用 `/playerLoadDistance help` 查看命令，末尾加 `persistent` 才会跨重启保存<br>非 OP 玩家的设置只影响自身的区块加载，不会再抬升全服视距；提高全服视距需要 OP<br>
...
### 食物清空命令权限(foodCommandPermission) · [相关指令](commands.md#cmd-food)

控制 `/food clear` 和 `/fga food clear` 的使用权限<br>
false：关闭命令<br>
true：允许所有玩家清空自己或指定玩家的饱食度和饱和度<br>
onlyself：非 OP 只能清空自己，OP 可清空指定玩家<br>
ops：需要 OP 2 及以上<br>
0-4：设置命令的最低权限等级

- 类型：`权限字符串`
- 默认值：`ops`
- 参考选项：`false`、`true`、`onlyself`、`ops`、`0-4`
- 分类：`FGA`、`特性`、`命令`
- 生效版本：`1.21.1`、`1.21.3`、`1.21.4`、`1.21.5`、`1.21.8`、`1.21.10`、`1.21.11`、`26.1.2`、`26.2`、`26.3`
- 客户端要求：无（纯服务端）
- 与其他规则的关系：无
...
让每名符合筛选条件的试炼参与玩家按指定人数计算，仅影响试炼刷怪和奖励规模<br>倍率对怪物生成规模完全生效，但单次结算的奖励弹射每名玩家最多 64 份，避免一次掉落上万物品<br>

...
$summary = Join-Path $logDir 'summary-1.18-1.20.6-retest.txt'
$versions = @('1.18.2','1.19.2','1.19.4','1.20.1','1.20.4','1.20.6')
...
foreach ($v in $versions) {
  ...
  Write-Host "==== SMOKE $v ===="
  $log = Join-Path $logDir ($v + '-smoke-retest.log')
  ...
  # force recompile of this version
  ...
  if (Test-Path $log) {
    ...
    $hasMixinFail = [bool]($raw -match 'Critical injection failure|Mixin prepare failed')

...
public final class EnchantmentLevelRules {
    ...
    public static int addLevels(int firstLevel, int secondLevel, int maximumLevel) {
        long sum = (long) firstLevel + secondLevel;
        return (int) Math.max(0L, Math.min(Math.min(MAX_STORED_LEVEL, maximumLevel), sum));
    }

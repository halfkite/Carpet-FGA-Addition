...
奪舍邏輯採用 PlayerControl 的線上實體狀態交換；被操控的假人使用 `/player <名字> kill` 時按原模組釋放會話並按原版結算，斷線、許可權收緊或規則關閉時由 FGA 結束會話<br>
...
僅在被操控者名稱後顯示[操控者]，方括號為紅色，操控者名稱為淺灰色
...
true 使用操控者自己的命令權限<br>
false 使用被操控者的命令權限<br>
...
自訂 `/player` 名稱參數的假人預設補全<br>
false：保留 Carpet 預設的 Steve 和 Alex<br>
填寫 `[名稱,名稱]` 後替換預設預設，並保留線上玩家與奪舍目標補全<br>
僅影響提示，不限制手動輸入
...
### 多人遊戲玩家無上限(unlimitedMultiplayerPlayers)

允許多人遊戲伺服器接納超過 `server.properties` 中 `max-players` 的玩家，不改變封禁、白名單和 IP 封禁檢查<br>
偵測到 GCA 已載入且 `fakePlayerResident` 開啟時自動生效
...
合成帶有附魔外觀的附魔金胡蘿蔔，可在飽食度滿時食用，食用後飽食度與飽和度均恢復到 20
...
任何群系的草方塊上用骨粉都能催出任意花種；花仍需光照充足才能生成，大花會生成完整兩格<br>
false：保持原版（只能催出當前群系的花）<br>
true：可催出原版本就能催熟獲得的所有小花<br>
all：還能催出大花（向日葵、丁香、玫瑰叢、牡丹）
...
百分之百催熟樹苗類作物、竹子、海泡菜等有機會一份骨粉生長多個階段的作物百分之百生長多個階段
...
- 生效版本：`1.21.1`
- 客戶端要求：無（純服務端）
- 與其他規則的關係：與 `grassBonemealAnyFlower` 獨立

### 命名終界珍珠傳送玩家(namedEnderPearlTeleport)

帶有自訂名稱的終界珍珠會將傳送目標改為同名的線上玩家，找不到同名玩家時該終界珍珠不執行傳送
...
### 工具火焰附加熔煉(fireAspectOnTools)

挖掘工具可以附魔火焰附加、火焰附加1會使掉落物熔煉1次、火焰附加2則是兩次；工具不會在附魔台上獲得火焰附加
...
### 靈魂疾行不消耗耐久(soulSpeedNoDurability)

啟用後靈魂疾行在靈魂沙、靈魂土等相容方塊上移動時不會消耗靴子耐久
...
村民新生成的附魔書會將抽到的附魔直接提升到目標等級<br>
onlyvanilla：提升到附魔書交易可達到的原版最高等級<br>
more：允許提升到 `enchantmentLevelLimitIncrease` 設定的等級<br>
僅影響村民重新取得職業或升級時新生成的交易
...
村民新生成的附魔武器和裝備會將抽到的附魔直接提升到目標等級<br>
onlyvanilla：按原版裝備交易的 5 至 19 附魔強度和物品附魔能力計算最高等級，不會把效率、鋒利等附魔直接提升到原版裝備交易無法達到的 V 級<br>
more：允許提升到 `enchantmentLevelLimitIncrease` 設定的等級<br>
未附魔裝備和其他交易不受影響，僅影響新生成的交易
...
啟用後透過 `/fga piglinBarterItemExclusions list` 查看並編輯目前豬布林交易戰利品表<br>
可以透過點擊列表內容進行編輯，機率總和不足 100% 時剩餘部分為空交易，超過 100% 時按比例分配
...
### 光源方塊切石配方(lightSourceStonecuttingRecipes)

啟用後可將信標、螢石、南瓜燈、海晶燈、蛙明燈、終界燭、火把、蠟燭等發光方塊放入切石機並切成亮度等級 1 至 15 的 `minecraft:light` 光源方塊，每次產出 4 個；`1.21.9+` 額外支援銅火把和各種氧化、塗蠟銅燈籠<br>
任意亮度等級的 `minecraft:light` 光源方塊也可以在切石機中互相一比一轉換
...
### 光源方塊可破壞(lightBlockBreakable)

生存玩家手持光源方塊可以破壞光源方塊<br>
客戶端需安裝 FGA 才能發起生存破壞

- 型別：`列舉`
- 預設值：`false`
- 參考選項：`false`、`true`、`onlyholding`
- 分類：`FGA`、`特性`
- 生效版本：`1.21.1`
...
### 地圖載入命令權限(mapLoadCommandPermission) · [相關指令](commands.md#cmd-map-load)

控制 `/mapLoad` 和 `/fga mapLoad` 的使用權限<br>
false：停用命令<br>
true：允許所有玩家使用<br>
ops：需要 OP 2 及以上<br>
0-4：設定命令的最低權限等級
...
### 食物清空命令權限(foodCommandPermission) · [相關指令](commands.md#cmd-food)

控制 `/food clear` 和 `/fga food clear` 的使用權限<br>
false：停用命令<br>
true：允許所有玩家清空自己或指定玩家的飽食度和飽和度<br>
...
- 型別：`權限字串`
- 預設值：`ops`
- 參考選項：`false`、`true`、`onlyself`、`ops`、`0-4`
- 分類：`FGA`、`特性`、`命令`
- 生效版本：`1.21.1`、`1.21.3`、`1.21.4`、`1.21.5`、`1.21.8`、`1.21.10`、`1.21.11`、`26.1.2`、`26.2`、`26.3`
- 客戶端要求：無（純服務端）
- 與其他規則的關係：無

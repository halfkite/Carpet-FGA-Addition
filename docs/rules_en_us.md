...
Possession uses PlayerControl's online entity state swap; the fake player's `kill()` path releases the session and follows vanilla death handling, while FGA ends sessions on disconnect, permission loss or rule shutdown<br>
...
Shows [controller] only after the controlled player's name, with red brackets and a light gray controller name
...
Lets bone meal grow any flower on grass blocks in any biome; flowers still need enough light to survive, and tall flowers generate both halves<br>`false`: keeps vanilla behavior (only the current biome's flowers)<br>`true`: grows any of the small flowers obtainable through vanilla bonemeal<br>`all`: additionally grows the tall flowers (sunflower, lilac, rose bush, peony)

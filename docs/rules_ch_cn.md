...
true 使用操控者自己的命令权限，false 使用被操控者的命令权限<br>
...
自定义 `/player` 名称参数的假人预设补全；`false` 保留 Carpet 默认的 Steve 和 Alex，填写 `[名称,名称]` 后替换这两个预设并保留在线玩家与夺舍目标补全，仅影响提示，不限制手动输入
...
产物使用原版金胡萝卜并带有附魔外观，可在饱食度满时食用，食用后饱食度与饱和度均恢复到 20<br>
false：关闭配方，已有附魔金胡萝卜按普通金胡萝卜的进食条件和恢复效果处理<br>
true：启用配方
...
村民新生成的附魔书会把抽到的附魔直接提升到目标等级；`false` 关闭，`onlyvanilla` 提升到附魔书交易可达到的原版最高等级，`more` 允许提升到 `enchantmentLevelLimitIncrease` 设置的等级；仅影响村民重新获得职业或升级时新生成的交易
...
村民新生成的附魔武器和装备会把抽到的附魔直接提升到目标等级；`false` 关闭，`onlyvanilla` 按原版装备交易的 5-19 附魔强度和物品附魔能力计算可达到的最高等级，因此不会把效率、锋利等附魔直接提升到原版装备交易无法达到的 V 级；`more` 允许提升到 `enchantmentLevelLimitIncrease` 设置的等级；未附魔装备和其他交易不受影响，仅影响村民重新获得职业或升级时新生成的交易
...
false：关闭自定义，完全使用原版交易<br>
true：启用交易物品的启用状态、交易概率和交易数量自定义<br>
列表按具体战利品变体显示客户端语言名称和英文 ID，每行提供概率、数量范围、删除或添加和重置操作<br>
概率总和不足 100% 时剩余部分为空交易，超过 100% 时按比例归一化<br>
旧版 `[ironBoots]`、`[potions]` 或物品 ID 列表会在首次加载时迁移到世界配置

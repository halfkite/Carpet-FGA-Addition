...
public final class FakePlayerItemSortCommand {
    ...
    public static com.mojang.brigadier.builder.LiteralArgumentBuilder<CommandSourceStack> root(String name){return Commands.literal(name).requires(s->CommandHelper.canUseCommand(s,CarpetSettings.commandPlayer))
            .executes(FakePlayerItemSortCommand::status).then(Commands.literal("status").executes(FakePlayerItemSortCommand::status))
            ...
            .then(Commands.literal("workers").then(Commands.argument("initial",IntegerArgumentType.integer(1)).then(Commands.argument("cached",IntegerArgumentType.integer(1)).executes(FakePlayerItemSortCommand::workers))))
    ...
    public static com.mojang.brigadier.builder.LiteralArgumentBuilder<CommandSourceStack> playerSort(){return Commands.literal("bot_sort").executes(c->sort(c,false)).then(Commands.literal("continuous").executes(c->sort(c,true))).then(Commands.literal("stop").executes(FakePlayerItemSortCommand::stop)).then(Commands.literal("restart").then(Commands.literal("all").executes(FakePlayerItemSortCommand::prepareRebuildAll).then(Commands.literal("confirm").executes(FakePlayerItemSortCommand::confirmRebuildAll))).then(Commands.argument("item",StringArgumentType.greedyString()).executes(FakePlayerItemSortCommand::rebuildOne)));}
    ...
    private static boolean chinese(){return "chinese".equals(FGASettings.fakePlayerItemSortTargetLanguage);}
    ...
    private static int status(CommandContext<CommandSourceStack> c){FakePlayerItemSortConfig.State s=FakePlayerItemSortConfig.snapshot();return ok(c,"mode="+FGASettings.fakePlayerItemSortMode+", whitelist="+FGASettings.fakePlayerItemSortWhitelist+", names="+s.names().size()+", "+FakePlayerItemSortManager.status());}

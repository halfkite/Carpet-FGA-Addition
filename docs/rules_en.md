...
`shulkerAttackArmorStand` is server-side only. Vanilla shulker targeting only covers players and `Enemy` mobs; armor stands are neither and are never targeted. This rule adds a low-priority goal: with `true`, every armor stand inside the targeting range (follow distance); with `pumpkin`, only armor stands whose head slot holds a carved pumpkin — survival players can equip one with a simple right click, no commands needed. Armor stands are detected exactly the way vanilla shulkers detect players (omnidirectional follow range plus line of sight), so floor-, ceiling-, and wall-attached shulkers all lock onto an armor stand just like they would a fake player. The vanilla `ShulkerAttackGoal` then aims and fires bullets as usual; player-first targeting, revenge targets, and the peaceful-difficulty check are all preserved (survival players within 16 blocks are targeted first, creative players never are). Rule changes apply immediately without restarts or re-summoning, and a dying or no-longer-matching armor stand target is released right away so targeting resumes. The legacy `onlyWithPumpkinHead` and `onlyWithShulkerShell` option names normalize to `pumpkin` automatically, so saved configs need no manual migration.

`resilientBlocks` is server-side only. Blocks in the list skip the vanilla `canSurvive` support check both on placement and during survival rechecks, so they can be placed in mid-air, on plants, or on any non-supporting block; they also ignore block updates: no `updateShape` self-checks (torch pop-off and similar), no own-state checks on neighbor updates (cactus, fences, and so on), and no fall scheduling on placement (floating sand and gravel stay put). The rule only touches listed blocks — anything else keeps vanilla behavior; remove IDs with `/carpet resilientBlocks []`. List entries are block IDs with the namespace optional (`sand` equals `minecraft:sand`); the stored value normalizes to a sorted list of full IDs.
...
On Minecraft `1.21.1` the rule has three modes: `only64` is the strict behavior above but judges full boxes and capacities by vanilla stack sizes (FGA stack limits ignored); `any` relaxes the input to any amount from 1 up to the container stack limit, converts the box content into craft count and crafts one click's worth, allows the final output box to be partially filled, leaves leftover material inside the input box, and converts it into an empty box when emptied to zero (empty boxes go to the inventory first, then drop); the legacy `true` value normalizes to `any`. In `any` mode all input boxes must contain the same number of items.

On `1.21+` the stonecutter is covered too. Place a shulker box of one material into the stonecutter input slot and the recipe list is resolved against the box content item (including this mod's multi-input `woodStonecuttingRecipes` such as bamboo x9); selecting a recipe shows one output box as the preview, and taking it (left click or Shift move) completes the whole transaction: box material is deducted by the recipe input count (leftovers stay in the box in `any` mode), the first output box is given with the take, and remaining output boxes and empty boxes go to the inventory first and drop when full. Stacked identical full boxes in the input slot are supported and consumed one per craft; in `only64` mode a recipe without a whole number of full output boxes produces no preview. The stonecutter recipe list is computed by the client menu: in singleplayer both sides agree naturally; on remote servers the client needs FGA installed with a matching local rule value to show the list (server-side deduction and output are always validated server-side and never trust the client).

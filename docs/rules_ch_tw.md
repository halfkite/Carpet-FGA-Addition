...
### 重生成後清除懸空蘑菇(terrainRegenerationRemoveFloatingMushrooms) · [相關指令](commands.md#cmd-regenerate-terrain)

地形重生成完成後，清除重生成區塊裡下方沒有方塊的蘑菇；下界荒地會用整列隨機高度放置蘑菇，重生成區域裡大量蘑菇會懸在空中，開啟本規則後只保留貼著方塊或緊貼方塊上方的蘑菇
false：保持原版結果，保留懸空蘑菇<br>
true：清除下方兩格內沒有方塊的蘑菇
...
- 生效版本：`1.21+`
- 客戶端要求：無（純伺服端）
- 與其他規則的關係：配合 `voidWorldGeneration` 使用；只影響地形重生成任務，不影響正常世界生成
...
帶有自訂名稱的終界珍珠會將傳送目標改為同名的線上玩家，找不到同名玩家時保持原版傳送行為
...
啟用後可將信標、螢石、南瓜燈、海晶燈、蛙明燈、終界燭、火把、蠟燭等發光方塊放入切石機並切成亮度等級 1 至 15 的 `minecraft:light` 光源方塊，每次產出 4 個<br>
